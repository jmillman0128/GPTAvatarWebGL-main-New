...
var MicrophoneNativeLibrary = {
    ...
    initSamplesMemoryData: function(byteOffset, length, left) {
        if(left === 0)
            document.microphoneNative.samplesMemoryDataLeftChannel = new Float32Array(HEAPF32.buffer, byteOffset, length);
        else
            document.microphoneNative.samplesMemoryDataRightChannel = new Float32Array(HEAPF32.buffer, byteOffset, length);
    },
    ...
    init: function(callbackJSON) {
        ...
        function unityCommand(command, object){
            ...
            // Unity 6^ support only
            {{{ makeDynCall('vi', 'ptrFunc') }}}(buffer);
