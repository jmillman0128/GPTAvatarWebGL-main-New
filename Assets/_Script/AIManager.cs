...
public class AIManager : MonoBehaviour
{
    ...
    public void SetActiveFriend(Friend newFriend)
    {
        ...
        // now set the character chosen in the webeditor on the server
        // this is right at the end of loading a config
        GameLogic.ViewCharacterBackground(_activeFriend); // step one, hide the background attached to the character model
    ...
    // we add the event listener a little bit later than awake
    // we want to make sure that the language selector (which registers in the awake method) has already
    // registered its own event listener -> then this is the second item in the event listener list
    // and should always be called a bit later :)
    void Start()
    {
        Debug.Log("Registering the callback function for the langauge change on the countryDropodown - class AI Manager");
        //countryDropdown.onValueChanged.AddListener(delegate { DropdownValueChanged(countryDropdown); });
    }
    ...
    // added function to change langauge (compare the class LanguageSelector, which contains the information
    // accessed here by LanguageSelector.SelectedGoogleVoice
    void DropdownValueChanged(TMP_Dropdown change)
    {
        if (_activeFriend != null && _activeFriend._googleVoice != null)
        {
            _activeFriend._googleVoice = LanguageSelector.SelectedGoogleVoice;
            Debug.Log("Setting the google voice in AIManager");
        }
        else
        { 
            Debug.Log("Problem with Dropdown in AI Manager or not running googleVoice. E.g. using Evelen labs which is not possible with this feature");
        }
    }
